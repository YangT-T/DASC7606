...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,time_embedding_dim):
        ...
        self.upsample=nn.ConvTranspose2d(in_channels=in_channels//2,out_channels=in_channels//2,kernel_size=2,stride=2)
        self.conv0=ResidualBottleneck(in_channels=in_channels,out_channels=out_channels)
        self.time_mlp=TimeMLP(embedding_dim=time_embedding_dim,hidden_dim=out_channels*2,out_dim=out_channels)
        self.conv1=nn.Sequential(*[ResidualBottleneck(out_channels,out_channels) for i in range(3)],
