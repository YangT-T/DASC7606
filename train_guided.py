...
from classifier_free_model import Diffuser
import math
import argparse
import datetime
...
def main(args):
    ...
    # point=torch.load("./logs/1022-141655/ckpt/57.pt")
    # model.load_state_dict(point["model"])
    # model_ema.load_state_dict(point["model_ema"])
    ...
    for i in range(args.epochs):
        ...
        training_progress = tqdm(
            train_dataloader, desc="Training Progress", leave=leave_option
        )
        for image, label in training_progress:
            ...
            training_progress.set_description(
                f"epoch-{i} loss: {loss.detach().cpu().item():.4f}"
            )
        ...
        model_ema.eval()
        samples = model_ema.module.sampling(args.n_samples, 1, device=device)
        save_image(
            samples, exp_path / "img" / f"{i}.png", nrow=int(math.sqrt(args.n_samples))
        )
