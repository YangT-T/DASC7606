...
class Diffuser(nn.Module):
    def __init__(
        ...
        betas = self._cosine_variance_schedule(
            timesteps
        )  # or self._linear_variance_schedule(timesteps)
    ...
    @torch.no_grad()
    def guided_sampling(self, n_samples: int, y, device="cuda") -> Tensor:
        x_t = torch.randn(
            (n_samples, self.in_channels, self.image_size, self.image_size)
        ).to(device)
        for i in range(self.timesteps - 1, -1, -1):
            noise = torch.randn_like(x_t).to(device)
            t = torch.tensor([i for _ in range(n_samples)]).to(device)
            x_t = self._guided_reverse_diffusion_with_clip(x_t, t, noise)
    ...
    def _forward_diffusion(self, x_0: Tensor, t: Tensor, noise: Tensor) -> Tensor:
        ...
        # ---------- **** ---------- #
        # YOUR CODE HERE
        alphas_cumprod_t = self.alphas_cumprod.gather(-1, t).reshape(
            x_0.shape[0], 1, 1, 1
        )
        sqrt_one_minus_alphas_cumprod_t = self.sqrt_one_minus_alphas_cumprod.gather(
            -1, t
        ).reshape(x_0.shape[0], 1, 1, 1)
        x_t = alphas_cumprod_t * x_0 + sqrt_one_minus_alphas_cumprod_t * noise
        return x_t
        # ---------- **** ---------- #
    ...
    @torch.no_grad()
    def _guided_reverse_diffusion_with_clip(
        self, x_t: Tensor, t: Tensor, noise: Tensor, y, scaler=3
    ) -> Tensor:
        """
        reverse diffusion process with clipping
        hint: with clip: pred_noise -> pred_x_0 (clip to [-1.0,1.0]) -> pred_mean and pred_std
              ...
              you may compare the model performance with and without clipping
        """
        guidance = self.classifier.get_guidance(y, x_t)
        ...
        return mean + guidance * scaler + std * noise
